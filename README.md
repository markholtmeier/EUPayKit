...
Drop-in iOS SDK for DMA-compliant alternative in-app purchases in the EU. Powered by EuroPay.
...
- Swift 5.9+
- Xcode 15+
- A [EuroPay](https://europay.dev) account with API key and app ID
...
<array>
    <string>applinks:europay.dev</string>
</array>
```
...
struct StoreView: View {
    @ObservedObject private var euroPay = EuroPayKit.shared!
    ...
    private func loadProducts() async {
        do {
            products = try await euroPay.fetchProducts()
        } catch {
    ...
    private func purchase(_ product: EuroPayProduct) async {
        do {
            ...
            let transaction = try await euroPay.purchase(
                product: product,
                userId: "user_123",
                presenting: vc
            )
...
```swift
let isEU = await euroPay.isEUUser()
if isEU {
...
```swift
try await euroPay.openCustomerPortal(
    userId: "user_123",
    presenting: viewController
)
```
...
| `baseURL` | `URL` | `https://api.europay.dev` | API base URL |
...
3. **Added Associated Domains** in your app's capabilities for `applinks:europay.dev`
4. **Registered a URL scheme** for checkout return callbacks
5. **Acknowledged the ~5% Core Technology Fee (varies by app)** Apple charges on alternative payment transactions
