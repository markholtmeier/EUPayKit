...
public final class EuroPayKit: ObservableObject {
    ...
    /// 2. Shows Apple's External Purchase disclosure (iOS 17.5+) or our custom DMA modal
    /// 3. Creates a Stripe Checkout session (with Apple token if available)
    ...
    public func purchase(
        ...
        // 2. Show Apple's External Purchase disclosure + capture token
        var appleToken: String? = nil
        if #available(iOS 18.1, *) {
            // Request external purchase token before showing the disclosure
            let token = try? await ExternalPurchaseCustomLink.token(for: "acquisition")
            appleToken = token?.value
            ...
            do {
                ...
                // User declined Apple's disclosure
                throw EuroPayError.userCancelled
            }
        } else {
            // Fallback: our custom DMA disclosure for older iOS (no Apple token)
            let userAccepted = await DMADisclosure.present(from: presenting)
            guard userAccepted else { throw EuroPayError.userCancelled }
        }

        // 3. Create checkout session (with Apple token)
        isLoading = true
        ...
        if let token = appleToken {
            body["appleExternalPurchaseToken"] = token
        }
        ...
        let result = try await EuroPayCheckoutSheet.open(
            url: checkoutURL,
        ...
        guard case .completed(let sessionId) = result else {
            // User cancelled — report null transaction if Apple token was captured
            if let token = appleToken {
                Task {
                    try? await network.post(
                        "/apple/report-null",
                        body: ["token": token]
                    ) as ReportNullResponse
                }
            }
            throw EuroPayError.userCancelled
        }
    ...
    public func openCustomerPortal(
        ...
        try await EuroPayCheckoutSheet.open(
            url: portalURL,
            mode: .inAppSafari,
            presenting: presenting
        )
